...
task="$(curl --header "${SONAR_HEADER}" --silent --fail --show-error --user "${SONAR_TOKEN}": "${ceTaskUrl}")"
...
qualityGateStatus="$(curl --header "${SONAR_HEADER}" --silent --fail --show-error --user "${SONAR_TOKEN}": "${qualityGateUrl}" | jq -r '.projectStatus.status')"
